...
def ejecutar_tarea(dataset_q, dataset_r, descriptores_r, filename_resultados, esCppWindows=False, esCppLinux=False):
...
def evaluar_tarea1(datasets):
    ...
    print("Evaluación Tarea 1 - 2024")
    # datos para la evaluacion
    dir_evaluacion = "evaluacion_t1"
    ...
    for name in datasets:
        ...
        print()
        print("------- EVALUACION EN: {} -------".format(nombre))
        t0 = time.time()
        f1 = evaluar_en_dataset(nombre, dir_evaluacion)
        segundos = time.time() - t0
        print("  tiempo: {:.1f} segundos".format(segundos))
        resultados[nombre] = (f1, segundos)
    # imprimir resultados
    print()
    print("--------------------------------------------")
    print("Resumen:")
    ...
    for nombre in resultados:
        ...
        f1s.append(f1)
        print("    F1-score en {}: {:.3f}   (tiempo={:.1f} segundos)".format(nombre, f1, segundos))
